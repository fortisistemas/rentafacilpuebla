<#macro nav>
<footer class="large-cont">
	<div class="container">
		<div class="row">
			<div class="col-xs-6 col-sm-6 col-lg-3">
				<h4 class="second-color">Contacto<span class="special-color">.</span></h4>
				<div class="footer-title-separator"></div>
				<p class="footer-p">siempre al pendiente de nuestros clientes nuestra prioridad es su bienestar</p>
				<address>
					<div class="footer-separator"></div>
					<span><i class="fa fa-envelope fa-sm"></i><a href="#">gerencia@rentafacilpuebla.com</a></span>
					<div class="footer-separator"></div>
					<span><i class="fa fa-phone"></i>22-21-66-79-72</span>
				</address>
				<div class="clear"></div>
			</div>
			<div class="col-xs-6 col-sm-6 col-lg-3">
				<h4 class="second-color">vinculos<span class="special-color">.</span></h4>
				<div class="footer-title-separator"></div>
				<ul class="footer-ul">
					<li><span class="custom-ul-bullet"></span><a href="/">Inicio</a></li>
					<li><span class="custom-ul-bullet"></span><a href="/search">Buscar</a></li>
					<li><span class="custom-ul-bullet"></span><a href="/contact">Contact</a></li>
				</ul>
			</div>
		</div>
	</div>
</footer>
<footer class="small-cont">
	<div class="container">
		<div class="row">
			<div class="col-xs-12 col-md-6 small-cont">
					<img src="/images/logo-light_rfp.png" alt="" class="img-responsive footer-logo" />
				</div>

			<div class="col-xs-12 col-md-6 footer-copyrights">
				&copy; Copyright 2015 <a href="http://themeforest.net/user/johnnychaos?ref=johnnychaos" target="blank">Jan Skwara</a>. All rights reserved. Buy on <a href="http://themeforest.net/user/johnnychaos/portfolio?ref=johnnychaos" target="blank">Themeforest</a>
			</div>
		</div>
	</div>
</footer>
</#macro>